...
//
// It reads nothing but the manifest, because the replay path runs it on
// manifests rebuilt from the database, where "we stored this" is no exemption.
// Each failure names its field: that string is all the operator gets to see.
